...
namespace PriorityQueue
{
	...
	/// </summary>
	/// <typeparam DisplayName="T">The values in the queue.  Must implement the PriorityQueueNode interface</typeparam>
		...
		where T : PriorityQueueNode
	{
		...
		/// </summary>
		/// <param DisplayName="maxNodes">The max nodes ever allowed to be enqueued (going over this will cause an exception)</param>
		...
		public void Clear()
		{
			// MEGA PERFORMANCE HIT FOR BIG HEAPS!!!
		...
		{
#if UNITY_EDITOR
			if (Contains(node)) throw new ArgumentException($"Node {node} is already in the queue!");
#endif
			_numNodes++;
